...
def project_to_bev(pcd_feat, des_coord, proj_H=512, proj_W=512, mode="max"):
    """
    Args:
        pcd_feat: [B*T, C, N, 1] - PointNet Feature
        des_coord: [B, T, N, 3, 1] - 데이터로더에서 Quantize된 데카르트 좌표
    """
    B, T, N, _, _ = des_coord.shape
    C = pcd_feat.shape[1]

    feat = pcd_feat.squeeze(-1).view(B, T, C, N)

    # 데이터로더 구조: [x_quan, y_quan, z_quan] 순서
    proj_x = des_coord[:, :, :, 0, 0].floor().long()  # [B, T, N]
    proj_y = des_coord[:, :, :, 1, 0].floor().long()  # [B, T, N]
...
def project_to_rv(pcd_feat, sph_coord, proj_H=64, proj_W=2048, mode="max"):
    """
    Args:
        pcd_feat: [B, C, N, 1] - 현재 프레임 PointNet Feature
        sph_coord: [B, T, N, 3, 1] - 데이터로더에서 Quantize된 구면 좌표
    """
    B, T, N, _, _ = sph_coord.shape
    C = pcd_feat.shape[1]

    feat = pcd_feat.squeeze(-1)

    # 데이터로더 구조: [theta_quan(H), phi_quan(W), r_quan] 순서
    # t0(현재 프레임, index: -1) 좌표만 사용
    proj_y = sph_coord[:, -1, :, 0, 0].floor().long()  # theta -> H
    proj_x = sph_coord[:, -1, :, 1, 0].floor().long()  # phi -> W
